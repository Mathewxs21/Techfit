...
class User(UserMixin):
    id : str
    def __init__(self, id, nome, email, senha, telefone, data_nascimento, genero, tipo_usuario, dias_treino, horario_treino, dias_trabalho, horario_trabalho, contador_dias = 0, contador_agua = 0):
        self.id = id 
        self.nome = nome
        self.email = email
        self.senha = senha
        ...
        self.contador_dias = contador_dias
        self.contador_agua = contador_agua
    ...
    def get(cls, user_id):
        ...
        if user:
            loaduser = cls(
                ...
                contador_dias=user['use_contador_dias'],
                contador_agua=user['use_contador_agua'])
    ...
    @classmethod
    def atualizar_foto_perfil(cls, user_id, foto_perfil):
        conn = obter_conexao()
        conn.execute('UPDATE users SET use_foto_perfil = ? WHERE use_id = ?', (foto_perfil, user_id))
        conn.commit()
